...
threadLocals变量是当前线程下的一个Map，Map的key是ThreadLocal实例，value就是我们放在ThreadLocal中的值。在多线程环境下。相当于是每一个线程都能够独立的拥有一个Map【因为map是Thread中的静态属性】。由于ThreadLocal是通过map获取值的。也就相当于是每一个线程中的ThreadLocal的局部变量的值都是相互独立的【虽然他们是同一个ThreadLocal对象，但是在get和set值的时候操作的都是map，而map是从Thread中取出来的】。
